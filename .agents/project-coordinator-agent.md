...
7. C# 게임플레이 코드/테스트 변경은 게임플레이 구현 에이전트에 배정한다.
8. 씬, 프리팹, 입력, 카메라, UI, Build Settings, ProjectSettings 변경은 Unity 씬/통합 구현 에이전트에 배정한다.
9. 코드, 씬, ProjectSettings, 승인 문서, 운영 문서 변경이면 QA/검증 에이전트와 프로젝트 총괄 관리자 게이트를 작업 순서에 포함한다.
10. 결과를 통합하고 사용자에게 승인 질문을 올린다.
11. QA/검증 기록과 총괄 관리자 판정이 없으면 완료 또는 커밋 가능 상태로 보고하지 않는다.
12. 완료 시 `_workspace/completed/<완료일>-<작업ID>/`에 완료 기록을 남긴다.
