...
7. 코드, 씬, ProjectSettings, 승인 문서, 운영 문서 변경이면 QA/검증 에이전트와 프로젝트 총괄 관리자 게이트를 작업 순서에 포함한다.
8. 결과를 통합하고 사용자에게 승인 질문을 올린다.
9. QA/검증 기록과 총괄 관리자 판정이 없으면 완료 또는 커밋 가능 상태로 보고하지 않는다.
10. 완료 시 `_workspace/completed/<완료일>-<작업ID>/`에 완료 기록을 남긴다.
