...
근본 교정된 자연 부분 가림 후보의 기술 검증을 유지하고, 실제 연속 WASD와 최종 화면 사용자 수용까지 이어서 확인한다.
...
- 다음 세션의 첫 목표: 총괄 1차 문서 정합 반려 뒤 read-only 재감사를 받고, 통과 시 사용자가 실제 WASD로 벽·통·상자 주변 이동과 자연 부분 가림·전체 소실 0을 확인한다.
