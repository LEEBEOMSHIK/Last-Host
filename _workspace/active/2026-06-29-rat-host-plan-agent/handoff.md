...
검토 중. 담당 에이전트 산출물 초안이 `artifacts/rat-host-implementation-plan-draft.md`에 작성되었다.
...
2. 조정자가 초안을 검토한다.
3. Unity 구조 검토가 필요한 항목은 Unity 아키텍처 에이전트에게 넘긴다.
4. 검증 기준은 QA/검증 에이전트에게 넘긴다.
5. 사용자 승인 후 공식 문서 승격 또는 구현 작업 패킷 생성 여부를 결정한다.
