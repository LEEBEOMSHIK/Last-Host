...
- 상태: Task 4 v11 상세 숏 명세 내부 승인 통과·`c50f71a` origin/main 반영 완료 — Task 5 사용자 결정 대기
- 생성일: 2026-08-08
...
| QA/검증 | 독립 정적 QA | 아래 C1~C14 및 변경 범위 확인 | `verification.md` QA 판정 |
...
- 총괄 관리자 판정 필요: 예
- 커밋 전 차단 조건 확인 필요: 예
- correction cycle: 2/2 — 기존 QA-021 기록 보정 1회에 더해 총괄 028의 파일 수명주기·러닝타임 이중 계산 blocker 2건을 단일 correction으로 보정했다. 기존 v1~v6 제작·기록 수정 주기 2/2는 별도 이력으로 보존한다.
...
| 편집 비트 후보·기원 공개 장부 | 픽셀아트 시네마틱 연출 에이전트 | 33개를 최종 숏 수로 확정하지 않고 Task 5 스토리보드·카메라·레이어·타이밍과 제작을 별도 승인 |
...
- 관련 suite: C1~C14 Markdown 정적 대조
...
- Task 5에서 유지·병합·분할할 편집 비트와 최종 스토리보드·카메라·레이어 명세
