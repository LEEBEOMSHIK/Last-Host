"""Render a 128px v4 copy of the read-only v3 rat walk asset in Blender.

v4 intentionally changes only the output canvas resolution.  Camera, lighting,
direction yaw, mesh deformation keys, timing, and root pivot remain v3-equivalent.
"""
import bpy
import csv
import json
...
TASK = os.path.join(ROOT, "_workspace", "active", "2026-07-21-character-sprite-resolution-standard")
INPUT_BLEND = os.path.join(
    ROOT, "_workspace", "active", "2026-07-20-rat-walk-animation-blender", "artifacts",
    "source", "rat-walk-trial-v3-exaggerated-motion.blend",
)
OUTPUT_BLEND = os.path.join(TASK, "source", "rat-walk-trial-v4-128px.blend")
RENDERS = os.path.join(TASK, "renders-v4")
FRAME_MAP = os.path.join(TASK, "walk-frame-map-v4.csv")
SETTINGS = os.path.join(TASK, "render-settings-walk-v4.json")
...
# The only output-quality change from v3: 2x linear pixels per canvas axis.
...
for frame in range(1, 9):
    ...
    for index, direction, yaw in DIRECTIONS:
        ...
        filename = "rat-walk-v4-f%02d-%02d-%s.png" % (frame, index, direction.lower())
        output_path = os.path.join(RENDERS, filename)
        ...
        records.append({
            ...
            "file": "renders-v4/" + filename,
            "phase": PHASES[frame],
        })

# Direction rotation never becomes authored movement, and the saved v4 begins at f01.
no_root_motion(root)
scene.frame_set(1)
...
settings = {
    "asset_id": "rat-walk-trial-v4-128px",
    "status": "trial-placeholder-not-final-art",
    "blender_version": bpy.app.version_string,
    "source": {
        "read_only_input": INPUT_BLEND,
        "output_blend": OUTPUT_BLEND,
        "root_pivot": [0, 0, 0],
        "source_forward": "+X",
        "content_change_from_v3": "output canvas resolution only",
    },
    ...
    "canvas": {
        ...
        "color_mode": "RGBA",
        "rendered_with": "bpy.ops.render.render(write_still=True)",
    },
    "unity_handoff": {"pixels_per_unit": 64, "custom_pivot_normalized": [0.5, 0.25], "pivot_px_top_left": [64, 96]},
    "animation": {
        "shape_keys": key_names,
        "frame_phase_sequence": [PHASES[frame] for frame in range(1, 9)],
        "root_location_scale_z_keyed": False,
        "root_transform_keyframes": root_fcurves,
    },
    ...
    "verification": {
        ...
        "all_png_dimensions": [128, 128],
        "all_png_channels": "RGBA",
        "per_key_min_z": key_minimums,
        "root_location": list(root.location),
        "root_scale": list(root.scale),
        "root_fcurves": root_fcurves,
        "loop_transition": "f08 -> f01",
        "camera_lighting_preserved_from_v3": True,
    },
}
...
print(json.dumps({"blend": OUTPUT_BLEND, "png_count": len(records), "size": [128, 128], "min_sole_z": min(key_minimums.values()), "root_fcurves": root_fcurves}, ensure_ascii=False))
