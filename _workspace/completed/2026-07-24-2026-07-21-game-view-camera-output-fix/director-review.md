...
## 재검토 — 실제 방향 이동 증적 갱신 (2026-07-23)

### 판정

**보류 유지**

D 및 W+D 입력 후 실제 Game 뷰에서 쥐·월드 구성이 변한 증적이 추가되어, 이전의 “입력 전송만 확인” 보류는 해소됐다. 다만 키 해제 뒤 화면·HUD 변화의 원인, 이동 중 쥐-카메라 상대 정합, 이번 세션 Console은 여전히 판정할 수 없으므로 작업 전체 완료·수용 또는 총괄 내부 승인 가능으로 변경하지 않는다.

### 갱신된 통과 범위

- Play 활성 증적이 있는 상태에서 기준→D 0.7초, D→W+D 0.7초의 쥐·초록 발판·주변 월드 화면 구성이 각각 변했다. **D 및 W+D의 실제 시각 이동은 통과**다.
- 키 해제 뒤 0.25초와 1.25초 캡처가 서로 다르고 HUD도 `하수도 탐색 중`에서 `소음 배관 조사 가능`으로 바뀌었다. **해제 뒤 화면 변화가 지속된 사실은 통과**다.

### 계속 보류인 항목

- 위 해제 뒤 변화가 카메라 보간, 키 해제 미수신, 쥐의 화면 밖 이동, 위험/탐색 자동 상태 전환 중 무엇 때문인지는 증적만으로 구분할 수 없다. 따라서 이전 세션의 1초 동일 캡처를 일반적인 idle 안정화 통과로 확대하지 않는다.
- 프레임 연속 영상·월드 좌표가 없으므로 D/W+D의 정확한 축/대각 벡터 및 이동 중 쥐-카메라 상대 지연·떨림 없음은 보류다.
- MCP `GetState` 재시도도 `Transport closed`이며 Console은 미검증이다. 이는 검증 인프라의 미복구 상태이지 새 게임 오류 증거는 아니다.

### 다음 단계

- native MCP transport 복구 후 QA가 Console과 입력 해제 직후의 쥐/카메라/게임 상태를 같은 Play 세션에서 확인하거나, 사용자가 D·W+D·키 해제 뒤 화면이 의도대로 안정되는지 직접 확인한다.
- 사용자 명시 커밋·푸시 예외는 기존 기록대로 유지하되, 이 재검토는 완료·수용 판정을 열지 않는다.

---

## 재검토 — 픽셀 누적 이탈 수정·숙주 본능 이동 복구·WASD 인계 독립 QA (2026-07-24)

### 검토 대상

- 사용자가 보고한 이동 시간 비례 RatHost–RatVisual 분리와 카메라 중심 이탈의 원인·수정 기록
- 쿼터뷰 즉시 추적, RatVisual 픽셀 스냅 앵커 수정, WASD 입력 우선, 무입력 숙주 본능 이동 복구 diff
- `task.md`, `work-log.md`, `agent-activity.md`, `verification.md`, `handoff.md`, `CURRENT.md`, 공유 현황판
- 현재 Git 상태와 범위 밖 `ProjectSettings.asset`, `_workspace/previews/` 구분

### 판정

**내부 승인 가능**

이번 판정은 **독립 QA의 기능 통과 결과를 사용자에게 보고할 수 있다**는 뜻이다. 물리 키보드 체감과 자연 시간 랜덤 휴지·회전 리듬은 사용자 최종 확인 항목으로 남으며, 최종 사용자 승인이나 커밋·푸시를 대신하지 않는다.

### 근거

- 수정은 승인된 쥐 숙주 프로토타입의 기본 이동, 고정 쿼터뷰, 8방향 시각 자식 정합 범위 안이다. 새 숙주·시스템·패키지·아트·ProjectSettings 변경을 추가하지 않았다.
- 픽셀 시험 이후의 누적 이탈은 자식 `RatVisual`의 이전 월드 위치를 다시 스냅하면서 부모 이동량의 반대 로컬 오프셋이 누적된 것이 원인으로 측정됐다. RatHost 루트를 매 프레임 원본 앵커로 사용하고 중복 시각 스냅을 끈 수정은 해당 원인에 직접 대응하는 최소 변경이다.
- 쿼터뷰 즉시 추적과 화면상 RatVisual 중심 초점 보정은 사용자가 보고한 카메라 지연·중심 이탈을 해당 모드 안에서만 고쳤다. 다른 카메라 모드, 960×540 Point 출력, 충돌·면역·변이 루프는 유지됐다.
- 무입력 숙주 본능 이동은 원인 분리 중 제거했던 기존 승인 범위 동작을 복구한 것이다. 활성 WASD에는 본능 벡터를 섞지 않고, 기존 반대 방향 강제 조종 감속·면역 패널티는 보존했다.
- 메인 에이전트 직접 구현은 사용자의 반복된 즉시 수정 승인과 2026-07-24 숙주 본능 재반영 요청이 있으며, `task.md`와 `agent-activity.md`에 예외 사유·변경 범위·보류 조건이 구체적으로 기록돼 있어 예외 게이트를 충족한다.

### QA/검증 기록 확인

- 독립 QA는 `RatHostPrototype` Play 진입, 시작 RatHost 모드, RatHost/RatVisual/IsometricCamera/HUD 연결을 확인했다.
- 무입력 360스텝에서 raw input은 0, 숙주 본능 이동은 360/360스텝 발생했다. RatHost–RatVisual XZ 최대 거리는 `0`, 화면 x 차이는 `0px`, 화면 초점 최대 오차는 `0.6748 internal-output pixel`이었다.
- Input System 합성 D/A/W/S는 `PrototypeKeyboardInput → RatHostController.Update → CharacterController.Move` 실제 게임 경로를 통과했다. 네 방향 모두 raw·해결·실제 이동 방향 내적이 `1`이고 첫 스텝은 설정값과 일치하는 `0.0416~0.064`여서 순간이동·본능 벡터 혼합이 재현되지 않았다.
- 키 해제 첫 스텝은 본능 0.45배 거리 `0.0288`로 복귀했고, 본능 휴지 모델은 무입력 정지와 입력 우선을 유지했다.
- 관련 스크립트 진단은 0건이었다. QA 도구가 만든 `TransformHandle` 직렬화 경고 2건을 분리·삭제한 뒤 게임 코드 Console Error/Warning은 0건이었고, Play 종료 후 씬은 dirty가 아니었다.

### MCP 플레이 체크 확인

- QA/검증 에이전트가 구현 주체의 사전검증과 분리해 Unity MCP Play를 수행했다.
- 실제 컨트롤러·시각 LateUpdate·카메라 LateUpdate와 Input System 입력 경로를 사용했고, Console 확인과 Play 종료까지 기록했다.
- 따라서 이전 보류 사유였던 이동 중 루트·시각·카메라 정합, D/A/W/S 방향·첫 이동량, 해당 세션 Console 공백은 MCP 대체 입력 범위에서 해소됐다.

### 미검증 항목 판단

- 물리 OS 키보드: 최종 변경 뒤의 OS 이벤트 검증은 아니지만, 2026-07-23 실제 W/D/W+D 화면 증적과 이번 독립 QA의 Input System 이후 실제 게임 경로 측정이 서로 보완한다. 이번 기능 판정을 막지 않으며 사용자 체감 확인으로 남긴다.
- 자연 시간 랜덤 휴지·회전: MCP 동적 반복 동안 자연 시간이 진행되지 않아 리듬은 미검증이다. 활성/휴지/입력 우선 분기와 경계 방향 전환은 각각 대체 검증됐으므로 구조적 정합 판정을 막지 않으며, 리듬 품질은 사용자 최종 체감 항목이다.
- EditMode 전체 Test Runner: 테스트 소스와 스크립트 진단, 실제 컨트롤러 360스텝, 네 방향 입력 경로 검증이 있다. 이번 제한된 회귀 QA 보고를 막지는 않지만, 다음 Test Runner 사용 가능 시 전체 실행 결과를 보강하는 것이 좋다.

### 상태판·Git·범위 경계

- `HEAD`와 `origin/main`은 `b6ad154`로 일치하며 공유 현황판 기록과 맞는다.
- 기능 변경 파일, active 작업 경로, 미커밋 상태가 현황판·CURRENT·handoff에 반영돼 있다.
- `UnityProject/ProjectSettings/ProjectSettings.asset`의 `APP_UI_EDITOR_ONLY` 자동 define과 `_workspace/previews/`는 범위 밖 변경으로 명시돼 있으며, 이번 수정·검증·판정 대상에 포함하지 않는다.
- 독립 QA 이후 현황판은 `기능 통과 — 총괄 재판정 대기`로 동기화됐고, 재현 근거가 사라진 `카메라 추적·픽셀 격자 보정안 결정 및 구현` 후보와 완료된 QA 후보가 제거됐다. 추가 보정 후보 제거는 현재 증거에 부합한다.

### 수정 필요

- 기능 QA 결과를 사용자에게 보고하기 전 추가 코드·씬 수정은 없다.
- 완료 보관 또는 커밋 전 조정자가 총괄 판정을 CURRENT·상태판에 반영하고, `task.md`의 커밋 전 체크리스트에 남은 `메인 에이전트 직접 구현 예외 사유 확인: 해당 없음` 문구를 실제 기록과 맞추는 문서 정합성 보완은 필요하다.

### 문제 사안

- 없음. 미검증 항목은 기능 실패나 승인 범위 충돌이 아니라 사용자 체감·검증 인프라 보강 항목이다.

### 사용자 결정 필요

- 실제 Game 뷰에서 무입력 랜덤 이동 리듬과 이동 도중 WASD 즉시 인계가 자연스러운지 최종 체감 확인.
- 체감 문제가 다시 재현되지 않으면 별도 카메라 추적·픽셀 격자 보정 작업은 열지 않고 현재 수정안을 유지한다.

### 사용자에게 올릴 핵심 확인 사항

- 무입력 본능 이동 중에도 쥐 시각과 카메라가 루트에서 분리되지 않았다.
- D/A/W/S는 첫 이동 스텝부터 의도한 카메라 상대 방향으로 인계됐고 순간이동이 없었다.
- 물리 키보드와 자연 시간 랜덤 리듬은 최종 사용자 체감 확인 항목이다.

### 다음 단계

1. 조정자가 이번 총괄 판정을 CURRENT·상태판과 커밋 전 체크리스트에 동기화한다.
2. 사용자에게 독립 QA 기능 통과와 남은 체감 항목을 보고한다.
3. 사용자가 체감을 수용하면 작업 종료·보관을 판단하고, 문제가 재현될 때만 별도 카메라·픽셀 보정 작업을 다시 연다.
