...
8. 이미지 생성이 필요하면 `docs/prototype/plans/rat-host-ai-assisted-art-workflow.md`와 ChatGPT 이미지 아트 에이전트의 승인·출처·선별 경계를 확인한다.

## 판단 기준

- 2026-07-27부터 Unity 기반 2D 아이소메트릭 도트 타일·방향별 캐릭터 스프라이트 방식이 확정 방향이다.
- 기존 3D 환경·저폴리 원본·Blender 프리렌더 파이프라인은 신규 제작 기본 경로가 아니라 레거시 이력이다.
- 목표 목업과 ChatGPT 이미지 결과는 reference·후보이며 실제 타일셋이나 최종 스프라이트로 자동 해석하지 않는다.
