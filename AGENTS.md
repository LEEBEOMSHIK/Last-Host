...
R1은 작업 패킷과 증거를 짧게 유지하고 불필요한 전문 역할·전체 suite·대형 matrix를 생략한다. 그러나 production 또는 운영 문서 변경의 독립 QA와 총괄 최종 판정은 생략하지 않는다. 새 상태 전이, 씬/ProjectSettings, 패키지, 범위 변경, 큰 리팩터링이 생기면 즉시 R2/R3로 재분류한다.
