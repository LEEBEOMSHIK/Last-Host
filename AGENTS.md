...
- 메인 에이전트가 직접 구현해야 하는 예외는 사용자 명시 승인과 `agent-activity.md`의 예외 사유 기록이 있을 때만 허용한다.
...
비용과 토큰 사용을 줄이기 위해 다음 조건을 모두 만족하는 작업은 경량 루프로 진행할 수 있다.
...
경량 루프에서도 필요하면 담당 에이전트에게 작업을 위임할 수 있다. 다만 `_workspace` 작업 패킷, 별도 서브에이전트 리뷰, 독립 QA 문서, 총괄 관리자 완료 문서는 생략할 수 있고 최종 보고에 변경 파일, 실행한 검증, 남은 위험만 짧게 적는다. 단, 승인 게이트를 새로 여는 작업, 씬/ProjectSettings 변경, 패키지 추가, 범위 변경, 큰 리팩터링, 커밋 요청, 실패 위험이 큰 작업은 기존 루프 엔지니어링 게이트를 따른다.
