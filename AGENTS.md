...
- 비주얼 방향은 3D 게임플레이 공간과 프리렌더드 캐릭터 스프라이트를 결합한 도트풍 하이브리드 2.5D다.
  - 환경·공간·충돌·카메라는 Unity 3D로 유지
  - 현재 승인된 쥐 프로토타입의 쥐·바이러스·백혈구는 저폴리 3D 원본에서 렌더한 8방향 스프라이트로 런타임 표시
  - 픽셀풍 표면과 제한 팔레트
  - 저해상도 렌더링 또는 후처리
  - 고정 쿼터뷰 또는 탑다운 카메라 구성
- 핵심 구조:
  - 숙주 조종 3D 탐험
  - 숙주 내부 바이러스 면역 미니게임
  - 변이 선택과 성장
- 첫 검증 목표: `쥐 숙주 프로토타입`
- 쥐 프로토타입 이후 숙주와 캐릭터에 같은 표시 방식을 확대할지는 별도 승인한다.
