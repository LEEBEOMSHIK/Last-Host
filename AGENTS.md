...
- 현재 단계: 쥐 숙주 프로토타입 구현 승인 완료. 승인된 범위 안에서 Unity 프로젝트 수정, 게임플레이 구현, 검증 작업을 진행할 수 있다.
...
- 사용자 보고에는 사용자가 직접 확인해야 하는 파일만 링크하고, 각 파일의 핵심 확인 사항을 함께 적는다.
- `_workspace` 전체 파일 목록은 이력/에이전트 작업 추적용으로 남기되, 사용자가 요청하지 않는 한 전체 확인 대상으로 제시하지 않는다.
...
- 2026-07-27 사용자 승인에 따라 비주얼·공간 표현의 현재 기준은 `2D 아이소메트릭/쿼터뷰 도트 게임`이다.
- 이 결정은 2026-07-16에 승인된 3D 환경·저폴리 3D 원본 기반 하이브리드 2.5D 방향을 대체한다. 기존 2.5D/Blender 산출물은 삭제하지 않고 레거시 이력으로 보관한다.
- 환경·캐릭터·효과는 2D 도트 타일과 스프라이트로 제작한다.
  - 환경과 이동 공간은 아이소메트릭 타일맵을 기준으로 구성
  - 쥐·바이러스·백혈구는 일관된 체형과 픽셀 밀도를 가진 방향별 2D 스프라이트로 표시
...
- Unity를 사용한다.
- Unity 프로젝트 루트는 `UnityProject/`이다.
- 1차 프로토타입은 Unity `6000.4.6f1`과 URP 사용이 승인되었다.
- 현재 3D/2.5D 구현은 2D 전환이 검증될 때까지 비교·회귀 확인용 레거시로 보존한다.
- 실제 2D 전환에서는 Tilemap, 2D 물리, SpriteRenderer/Animator, Y축 깊이 정렬, 픽셀 카메라 기준을 별도 기술 샘플로 검증한다.
...
- 모든 역할을 관성적으로 호출하지 않는다. R0~R3 위험 등급과 변경 영향에 필요한 최소 역할·기록·검증만 배정한다.
- 루프 적용 중 범위 충돌, 승인 게이트, 검증 불가, 에이전트 산출물 충돌, 작업 기록 누락이 생기면 사용자에게 문제 사안으로 보고한다.
- 코드, 씬, ProjectSettings, 승인 문서, 운영 문서 변경은 완료 또는 커밋 보고 전 위험 등급별 canonical 기록과 필요한 QA·총괄 판정이 있어야 한다.
- Unity 플레이어블 변경은 사용자 최종 확인 전 QA/검증 에이전트가 가능한 범위의 Unity MCP 플레이 체크를 수행하거나 수행 불가 사유를 기록한다.
- 프로젝트 총괄 관리자 에이전트는 MCP 플레이 체크 실행자가 아니라 QA/검증 기록 확인과 내부 승인 판정 담당이다.
...
- 2026-08-06 이후 새 작업은 R0 무기록, R1 `record.md` 1개, R2/R3 `task.md`+`verification.md`를 기본으로 한다. R3 분리 이력·활동·완료 보고, `handoff.md`, `artifacts/`는 기본 파일에 안전하게 통합할 수 없는 실제 추적·인계·증거가 있을 때만 만들며 기존 이력은 소급 변경하지 않는다.
- 위임 대상 에이전트가 정의된 작업은 조정자가 최종 산출물을 직접 만들기 전에 `_workspace/active/<작업ID>/` 작업 패킷을 먼저 만든다.
- 작업을 여러 에이전트로 나눌 때는 반드시 작업 목적, 입력 자료, 산출물, 금지 범위, 승인 필요 항목을 함께 전달한다.
- 에이전트 산출물은 바로 적용하지 말고, 담당 에이전트 또는 조정자가 검토한 뒤 반영한다.
- 목업·콘셉트·스프라이트 초안처럼 생성 이미지가 필요한 작업은 ChatGPT 내장 이미지 생성을 우선 연계할 수 있다. 생성 결과와 프롬프트·선별 근거를 작업 패킷에 기록하고, 사람의 승인과 게임 규격 재제작·QA를 거치기 전에는 최종 에셋으로 선언하지 않는다.
- 새 Codex 스킬을 추가로 생성하기 전에는 스킬 이름, 생성 위치, 포함 리소스, 검증 방법을 사용자에게 승인받는다.

## 저위험 구현 경량 루프

비용과 토큰 사용을 줄이기 위해 다음 조건을 모두 만족하는 작업은 `loop-engineering-gates.md`의 R1 요약형 루프로 진행할 수 있다.

- 이미 사용자 승인된 현재 구현 범위 안의 작은 수정이다.
  - 예: 현재는 쥐 숙주 1차 프로토타입 승인 범위 안의 작은 수정.
- 변경 파일이 적고, 새 패키지/에셋/씬 구조/ProjectSettings/에이전트 역할/승인 문서를 바꾸지 않는다.
- 실패해도 저장소 구조나 플레이어블 루프 전체를 크게 깨뜨리지 않는다.
- 사용자가 "경량", "빠르게", "비용 줄여서" 같은 의도를 밝혔거나, 조정자가 명확히 저위험으로 판단할 수 있다.

R1은 `record.md` 하나와 표적 검증 1회만 기본으로 한다. 실행 코드·자동화 스크립트, 사용자 동작·데이터, 공개 계약, 검증 신뢰성, 파괴 가능 작업 영향은 독립 QA를 유지하고, 실행 결과에 영향 없는 오탈자·링크·색인·표현 정리만 생략할 수 있다. 범위·승인 충돌은 총괄 감사와 사용자 승인을 거치며, 불명확하거나 새 상태 전이·씬/ProjectSettings·패키지·큰 리팩터링이 생기면 R2/R3로 재분류한다.
