...
- [x] **Step 1:** 2026-08-10 사용자 승인에 따라 첫 제작 범위를 `A01` 회사 기준 숏의 16:9 후보 3개로 제한한다. 출력은 기존 오프닝 작업 패킷의 `artifacts/task6/a01-office-base/`에 두며, 감염 징후·기침·마스크·보라색 파지 모티프·공포·텍스트·로고·특정 작품 스타일 복제를 금지한다.
- [ ] **Step 2:** ChatGPT 이미지 아트 담당이 숏 또는 레이어별 후보 2~3개를 생성한다. (`A01` 3개 생성 완료, 이후 숏은 사용자 선별 대기)
- [x] **Step 3:** `A01` 3개의 프롬프트·날짜·입력 reference·출력 경로·해시를 단일 생성 로그에 기록했다. 이후 생성 묶음도 같은 로그 규칙을 따른다.
- [ ] **Step 4:** 사용자가 후보를 선택하고 비주얼/테크아트가 픽셀·팔레트·실루엣 적합성을 검토한다.
- [x] **Step 5:** 생성된 `A01` 3개를 모두 `미선별` 후보로 유지하며 Unity에 반입하지 않는다.

## Task 7: 비최종 애니매틱과 오디오 큐

**Files:**

- Create after Gate M approval: `a01-office-hybrid-motion-design.md`, 비최종 애니매틱 결과 경로
...
- [x] **Step 1:** 2026-08-10 사용자 승인에 따라 `A01`을 혼합형 모션으로 진행한다. 후보 03의 공간과 후보 02의 연기를 결합하는 장면별 제작 명세를 작성하되, 실제 Unity 프리비즈는 명세 검토 뒤 별도 구현 계획으로 넘긴다.
- [ ] **Step 1a:** 사용자가 `a01-office-hybrid-motion-design.md`의 비트·레이어·연속성 앵커를 검토한다.
- [ ] **Step 1b:** 검토된 명세를 바탕으로 독립 `A01` 프리비즈의 Unity 파일 경로·레이어 제작·테스트 구현 계획을 확정한다.
- [ ] **Step 2:** `A01`의 깨끗한 배경·인물·전경 레이어를 준비하고, 제한 프레임 표정·몸짓과 카메라를 독립 프리비즈로 연결한다.
- [ ] **Step 3:** 무음 상태에서 발화 주체·지연 반응·함께 웃음·점심 이동 준비와 픽셀 안정성을 검토한다.
- [ ] **Step 4:** 사용자가 실제 `A01` 재생 결과의 연기·구도·호흡을 검토한 뒤 유지 길이를 측정·조정한다.
- [ ] **Step 5:** 환경음·웃음·의자·발걸음의 임시 큐는 별도 `Gate AU` 뒤 배치하고 실제 음원·성우 결과로 승인하지 않는다.
- [ ] **Step 6:** `A01`이 승인된 뒤에만 `A02` 또는 도시 일상 전체 연결을 별도 범위로 확장한다.
