...
| `2026-08-02-production2d-natural-occlusion-root-fix`<br>Production2D 자연 부분 가림·실제 충돌 루트 교정 | R2 — 사용자 가시 상태와 runtime/scene 결합 | 조정1, gameplay1, scene1, docs1, 독립 QA1, 총괄1.<br>S0→gameplay targeted→scene integration→S1~S5→freeze→S6/S7 | 조정1, visual footprint analyst1, gameplay1, scene1, docs1, stale fixture test owner1, 독립 QA1.<br>총괄 최종 판정·사용자 수용 대기 | S0 문서 QA 4회(3 FAIL·1 PASS), static measurement1.<br>gameplay `3/3` 1회, scene `7/8`→`8/8` 2회, stale fixture `4/4` 1회 | 결과 XML 기준 격리 Unity test run 6개(gameplay1·scene2·fixture1·full2).<br>MCP TestRunner no-result 요청 2, QA Play session 3(r1 partial·r2 command 거부·r3 PASS), build 0 | **2회**: r1 `200/203 FAIL` stale contract, r2 `203/203 PASS` final.<br>같은 fingerprint 중복 아님 | reduced Play r1 1회 partial, r3 공개 API 정렬 harness 1회 PASS.<br>r2 RunCommand 실행0, capture 0 | S0 r1/r2 2/2→재분류, r3→r4 해소.<br>scene correction 1/2, QA Play correction 2/2, stale fixture correction 2/2 종료 | S0 r1/r2/r3, scene r1, QA Play r1/r2, full r1은 historical/SUPERSEDED.<br>MCP no-result 2와 QA r2 Play/no-command 1 | **기술 후보 `4cb578b` 원격 보존 — 과다, 부분 회피 가능**<br>no-result Unity/MCP와 QA 하네스 correction 2회가 과다 기준에 해당.<br>정확 token/금액 미집계 | exact footprint 계약·계측, gameplay/scene 구현과 targeted, initial barrel 침투 correction, stale fixture targeted, blocker 발견 full r1+final full r2, 공개 API Play r3, final read-only evidence audit | stale owner/oracle 문구 보정 반복, MCP no-result 2건, QA r1 Rigidbody/Transform self-check 누락과 r2 reflection 권한 blocker로 발생한 Play 2회. final audit는 동적 실행 0으로 제한 | 기능 커밋 `4cb578b fix: implement natural 2d object occlusion` origin/main 반영 완료<br>`_workspace/active/2026-08-02-production2d-natural-occlusion-root-fix/`<br>canonical `natural-occlusion-final-evidence-r1-20260802` | 2026-08-03 KST |
