...
비활성 상태로 둔 이유는 아직 Unity 프로젝트와 `com.gamelovers.mcp-unity` 패키지가 설치되지 않았기 때문이다. 지금 활성화하면 Codex 시작 시 서버 파일을 찾지 못해 MCP 연결이 실패할 수 있다.
...
프로젝트 로컬 Codex 설정은 패키지 캐시 해시가 바뀌어도 동작하도록 PowerShell 래퍼가 서버 파일을 탐색하게 구성했다.

## 활성화 절차

다음 절차는 사용자 승인을 받은 뒤 진행한다.

1. Unity 프로젝트 생성 승인을 받는다.
2. Unity 프로젝트를 이 저장소 안에 생성한다.
3. Unity Package Manager에서 `https://github.com/CoderGamester/mcp-unity.git` 패키지 추가 승인을 받는다.
4. 패키지를 설치한다.
5. Unity 메뉴에서 `Tools > MCP Unity > Server Window`를 연다.
6. `Force Install Server`를 실행한다.
7. `Start Server`로 Unity MCP 서버를 시작한다.
8. `Library/PackageCache/com.gamelovers.mcp-unity@*/Server~/build/index.js` 파일이 있는지 확인한다.
9. `.codex/config.toml`에서 `enabled = true`로 바꾼다.
10. Codex를 프로젝트 신뢰 상태로 다시 열고 `/mcp`로 `mcp-unity` 연결을 확인한다.
