...
- 위험도와 변경 영향에 필요한 최소 역할·기록·검증만 투입한다.
...
| R1 국소 수정 | 기존 계약 안의 문구·값·1~3개 파일 국소 코드 | 구현 소유자 1명. 위험 production만 독립 QA, 범위·승인 충돌만 총괄 감사 | `record.md` 1개 | 표적 검증 1회. 실행 결과에 영향 없는 오탈자·링크·색인·표현 정리만 QA 생략 |
| R2 상태/통합 수정 | 새 상태 전이, 씬+코드 결합, 물리·가림·정렬·직렬화, 사용자 가시 결함 | 조정자, QA S0, 불변식별 구현 소유자, 필요 전문 역할, 독립 QA, 총괄 | `task.md`, `verification.md` | S0~S5 단계 검증 후 후보 freeze. QA가 필요한 전체 suite를 canonical 1회, 필요한 수용 증거 1회 실행 |
| R3 구조/릴리즈 | 패키지, ProjectSettings, 렌더 파이프라인, 저장 형식, 대규모 씬·아트, 릴리즈 빌드 | R2 역할 + 구조·통합·릴리즈 위험에 필요한 전문 역할, 사전 총괄 검토 | 기본 `task.md`, `verification.md`; 분리 추적이 실제 필요한 경우만 조건부 파일 | S0~S7 전체. 빌드·전체 회귀는 freeze된 후보에서 QA canonical 1회씩, 승인 범위까지 대조 |

파일 수가 적어도 새 상태, 직렬화, 사용자 화면, 복구 난이도가 커지면 R2/R3가 될 수 있다. 구현 중 예상 밖의 상태·설정·담당 밖 production 파일이 생기면 즉시 멈추고 재분류한다.

이 구조는 2026-08-06 이후 신규 작업에만 적용한다. R3도 기본은 `task`와 `verification`이며, `work-log`·`agent-activity`·`completion-report`는 기본 파일에 안전하게 통합할 수 없는 실제 다중 소유권·규제/릴리즈 추적이 있을 때만 추가한다. `handoff`는 세션 중단·외부 차단·실제 인계 때만, artifact는 원래 위치 참조만으로 부족한 indispensable canonical 증거가 있을 때만 추가한다. 기존 이력은 바꾸지 않는다.
...
이 계약은 검증 진실성을 줄이지 않는다. production 단일 owner, 증상 은폐 금지, attempt ledger, candidate fingerprint, canonical run, `SUPERSEDED`, lease, correction 2회 재분류를 유지한다. S0는 실행 전 계약 검토이고 preflight 차단은 실제 고비용 실행이 아니다.
...
R2/R3 또는 실제 고비용 실행이 발생한 작업의 비용은 [`task-cost-dashboard.md`](../project-handoff/task-cost-dashboard.md)에서 확인한다. R0과 고비용 없는 R1은 행을 만들지 않는다. 정확한 토큰·금액은 플랫폼 계측이 있을 때만 본다.
...
- [ ] 등급·변경 영향에 필수인 구현자, 독립 QA, 총괄 책임이 canonical 기록에 있는가?
...
- [ ] 비용 중앙 현황판 대상 작업이면 필요한 시점에 갱신됐는가?
...
| 2026-08-05 | preflight·S0와 실제 고비용 실행 구분, 구현·QA 재진입 상한, 상태-only 최종 동기화와 사용자 보고 압축 계약 추가 | 독립 정적 QA 1회·총괄 read-only 1회 완료, 운영 적용 |
| 2026-08-06 | 신규 작업의 R0 무기록·R1 단일 기록·R2/R3 기본 두 파일, 조건부 분리 기록·인계·artifact와 completed 동일 최소 폴더 이동 규칙 반영 | 후보 구현, 독립 QA·총괄 대기 |
