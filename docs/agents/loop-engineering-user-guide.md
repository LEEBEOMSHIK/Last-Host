...
이 계약은 내부 추적성을 줄이지 않는다. attempt ledger, candidate fingerprint, canonical run, `SUPERSEDED`, lease와 비용 기록은 기존 규칙대로 유지한다. S0는 실행을 시작하기 전의 계약 검토이고, preflight 차단은 실제 고비용 실행이 아니다.
