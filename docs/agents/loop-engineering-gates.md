...
검증 진실성과 기록 파일 수는 분리한다. 필요한 독립 QA와 총괄 감사는 유지하되 위험에 필요한 역할만 호출하고, 싸고 직접적인 검증부터 실패 즉시 중단한다. 아래 경량 기록 구조는 2026-08-06 이후 새로 시작하는 작업에만 적용하며 기존 `active/`·`completed/` 이력은 소급 변환하지 않는다.
...
| R1 국소 수정 | 기존 계약 안의 문구·값·국소 코드, 새 상태·공개 API·에셋·설정 없음, 보통 1~3개 파일 | 구현 소유자 1명 → 표적 검증 1회. 위험 production이면 독립 QA 1회, 범위·승인 충돌이면 총괄 감사와 사용자 승인 대기 |
...
- R1에서 실행 코드·자동화 스크립트, 사용자 동작·데이터, 공개 계약, 검증 신뢰성 또는 파괴 가능 작업에 영향을 주는 production 변경은 독립 QA가 필수다. 실행 결과에 영향 없는 오탈자·링크·색인·표현 정리만 QA를 생략할 수 있으며 불명확하면 R2로 올린다.
- R2/R3의 독립 QA와 최종 총괄 판정은 유지한다. R1 총괄 감사는 범위·승인 충돌이나 불명확성이 있을 때만 필수이며 사용자 승인을 대신하지 않는다.
...
R1은 `_workspace/templates/record.md` 하나를 사용한다. 다음 최소 필드만 요구한다.
...
6. QA·총괄 적용 여부와 필요한 판정

R1은 별도 `work-log.md`, `agent-activity.md`, `verification.md`, `completion-report.md`를 만들지 않는다. 정식 상태 전이표, 수명주기 matrix, 전체 S0 criterion 표, 전체 suite·대형 matrix 계획이 필요하거나 새 상태·직렬화·씬 통합이 드러나면 구현을 중지하고 R2로 재분류한다. 위험 production의 QA는 원증상·완료 주장·표적 검증을 현재 후보에서 독립 확인하고 같은 `record.md`에 판정을 남긴다.
...
| 등급 | 신규 작업의 canonical 기록 | 조건부 기록 |
| --- | --- | --- |
| R0 | 없음 | 없음 |
| R1 | `record.md` 1개 | 없음. 필요한 QA·총괄 판정도 같은 파일에 기록 |
| R2 | `task.md`, `verification.md` | `handoff.md`는 세션 중단·외부 차단·실제 인계 때만, `artifacts/`는 실제 증거가 있을 때만 |
| R3 | `task.md`, `verification.md` | 다중 production owner·규제/릴리즈 추적 등 기본 파일에 안전하게 통합할 수 없는 경우에만 `work-log.md`·`agent-activity.md`·`completion-report.md`; `handoff.md`와 `artifacts/`는 R2와 같은 조건 |

R2/R3의 `verification.md`가 실제 수행 이력, 독립 QA, 총괄 판정, canonical evidence와 최종 상태를 함께 소유한다. R3 분리 파일은 실제 책임 분리나 릴리즈 추적을 기본 두 파일에 안전하게 통합할 수 없을 때만 만든다. 빈 폴더·빈 템플릿·에이전트별 원문 보고서·중복 증거·상태만 다른 세대 파일은 만들지 않는다. 기록 위치가 줄어도 production 단일 owner, 증상 은폐 금지, 후보 변경 시 PASS 무효화, correction 2회 중단·재분류, 고비용 preflight와 미검증 완료 금지는 모든 변경 등급에 그대로 적용한다.
...
3. **독립 QA 상한**: 구현자의 current fingerprint가 green이 된 뒤 독립 QA가 1회 진입한다. QA 실패를 소유자가 보정한 뒤 QA 재진입은 1회까지만 허용하며, 두 번째 QA 실패에서는 중지·재분류·사용자 보고한다. 이 상한은 해당 등급·변경에 필수인 독립 QA의 생략을 허용하지 않는다.
...
정확한 토큰 수와 금액은 플랫폼이 작업별 계측값을 제공할 때만 기록한다. 계측값이 없으면 추정값·가상 금액·절감률을 만들지 않는다. 중앙 비용 현황판은 R2/R3 또는 실제 Unity/MCP/build/full suite/matrix/capture 같은 고비용 실행이 발생한 작업만 대상으로 한다.

- 작업 시작: 대상 작업이면 조정자가 R등급, 계획 역할·인계, 표적 검증, 고비용 실행과 artifact 예산으로 중앙 행을 만든다. R0과 고비용 실행 없는 R1은 행을 만들지 않는다.
...
- R1의 필요한 비용 정보는 `record.md`에 짧게 남긴다. 실제 고비용 실행이 생기면 중앙 행도 추가한다. R2/R3는 `task.md`의 계획과 `verification.md`의 실제 근거를 사용하며 별도 per-task 비용 파일은 만들지 않는다.

비용 `과다`는 실패나 미검증을 자동 통과시키는 예외가 아니다. 원인·회피 가능 비용을 공개하고, correction 2회 조건이면 재분류가 끝날 때까지 다음 고비용 실행과 완료·커밋을 차단한다.

## 완료 전 필수 게이트

완료·커밋 전에는 위험 등급에 맞는 다음 게이트를 충족해야 한다.

1. **R1**: `record.md`에 원증상·완료 주장·등급 근거·owner와 변경 파일·금지 범위·표적 검증·correction·QA/총괄 적용 여부와 최종 상태가 있다. 필수 조건에 해당하면 독립 QA 또는 총괄 판정도 같은 파일에 있다.
2. **R2**: `task.md`에 정식 S0와 소유권이 있고, `verification.md`에 실제 수행·독립 QA·총괄 판정·현재 canonical evidence·최종 상태가 있다.
3. **R3**: `task.md`와 `verification.md`, 조건을 충족해 실제로 생성한 분리 파일·artifact가 있으며 독립 QA와 총괄 판정이 현재 revision을 지지한다.
4. **공통**: 필요한 사용자 수용이 남으면 `완료`로 표현하지 않는다. 대상 작업만 중앙 비용 현황판을 갱신하고, 공유 상태판은 현재 active/next 후보와 실제 Git 상태가 필요할 때 동기화한다.
...
- 위험 등급별 필수 기록이나 production 소유자가 없고, 해당 등급·변경에 필수인 독립 QA 또는 총괄 판정이 없다.
...
- 중앙 비용 현황판 대상인데 행이 없거나 blocker·correction·사용자 보고·커밋 전 실제값과 판정이 동기화되지 않았다.
