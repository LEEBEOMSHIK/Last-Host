...
- 초안의 핵심 방향은 도트풍 저폴리 3D 진화 생존 로그라이크이며, 현재 작업 해석본은 3D 원본 기반 캐릭터 스프라이트를 결합한 하이브리드 2.5D 제작 방식으로 구체화되었다.
- 초안은 핵심 루프 검증을 위해 `쥐 숙주 프로토타입`부터 시작하는 방향을 제안한다.

## 확정된 결정

- 엔진: Unity 3D
- 비주얼: 순수 2D 도트가 아닌 도트풍 하이브리드 2.5D
- 렌더링 방향:
  - 환경·공간·충돌·카메라는 Unity 3D 유지
  - 현재 쥐 프로토타입의 쥐·바이러스·백혈구는 저폴리 3D 원본에서 렌더한 8방향 스프라이트로 표시
  - 픽셀풍 표면과 제한 팔레트
  - 저해상도 렌더링 또는 후처리
- 게임 구조:
  - 바깥 세계의 숙주 조종 탐험
...
- 현재 3D 플레이스홀더는 실제 8방향 스프라이트 통합 전까지 게임플레이 검증용으로 유지한다.
- 쥐 프로토타입 이후 숙주와 캐릭터의 표시 방식은 후속 마일스톤에서 별도 승인한다.
